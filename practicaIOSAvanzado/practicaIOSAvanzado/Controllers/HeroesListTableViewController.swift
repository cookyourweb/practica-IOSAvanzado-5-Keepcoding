...
class HeroesListTableViewController: UIViewController {
    ...
    private func getFullHeroeApiClient() {
        if getHeroesCoreData().isEmpty {
            ...
            mainView.loadingIcon.startAnimating()
            ...
            let moveToMain = { (heroes: [HeroModel]) -> Void in
                ...
                self.mainView.loadingIcon.stopAnimating()
            }
  ...
  @objc
    func closeTapped(_ gestureTap: UITapGestureRecognizer) {
            self.dismiss(animated: true)
debugPrint("cierra ventanica")
     presentLoginViewController()
